...
static void vkprintf(__m256d a) {
    ...
    printf("%5.2lf %5.2lf %5.2lf %5.2lf ", tmp[0], tmp[1], tmp[2], tmp[3]);
}
...
int main(int argc, char* argv[]) {
    ...
    __mmask8* masks     = (__mmask8*) malloc(sizeof(__mmask8) * op.in_channels * op.kernel_size * op.kernel_size * op.out_w * op.out_h);
    int*    masks_off   = (int*) malloc(sizeof(void*) * op.in_channels * op.kernel_size * op.kernel_size * op.out_w * op.out_h);
    int masks_idx;

    img2col_zcomp(img_m, out_m, &op, masks, &masks_idx, masks_off);

    //* perform matrix multiply
    int ok = _mm_avx256m_zcomp(res, out_m, wei_tr_m, op.out_w*op.out_h, op.in_channels*op.kernel_size*op.kernel_size, op.in_channels*op.kernel_size*op.kernel_size, op.out_channels,
                    masks, masks_idx, masks_off);
    if(ok)  {
        printf("errno: %d\n", ok);
        return ok;
    }
