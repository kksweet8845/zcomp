...
int main(int argc, char* argv[]) {
    ...
    double ** out = malloc(sizeof(double*) * op.out_w * op.out_h);
    ...
    int ok = _mm_avx256m(res, out_m, wei_m, op.out_w*op.out_h, op.in_channels*op.kernel_size*op.kernel_size, op.in_channels*op.kernel_size*op.kernel_size, op.out_channels);
    if(ok)  {
        printf("errno: %d\n", ok);
        return ok;
    }
